...
def UPDATE_BOOKS_DETAILS(request):
    if request.method == 'POST':
        ...
        isbnnum = request.POST.get('isbnnum')
        price = request.POST.get('price')
        ncopies = request.POST.get('ncopies')  # New copies
        bookimage = request.FILES.get('bookimage')

        try:
            # Fetching the book, category, and author objects
            ...
            if ncopies.isdigit():  # Ensuring it's a valid number
                books.acopies += int(ncopies)

            # Updating book image if provided
            if bookimage:
            ...
            messages.error(request, "Invalid ID provided for book, author, or category.")
            return redirect('update_books')
    
    # Render the update_books template if the request is not POST
...
#         issuedcopies = request.POST.get('issuedcopies')
#         try:
...
def ISSUE_BOOK(request):
    if request.method == 'POST':
        ...
        try:
            # Fetch the Book and Student instances
            ...
            return redirect('issue_book')  # Redirect to the issue book page
        except Exception as e:
            messages.error(request, f'Error issuing book: {e}')
            return redirect('issue_book')  # Redirect back on error

    else:
        # Get all students
        students = Student.objects.all()

        # Fetch books that are available to issue (filtering based on isIssued or available_copies)
        books = Book.objects.filter(Q(isIssued=False) | Q(acopies__gt=0))
